...
def test_folder_input_planner_batches_all_subfolder_groups(tmp_path: Path) -> None:
    ...
    diff_1 = write_tiny_wav(folder / "TSSS 完パケ音声 mp3" / "差分トラック mp3" / "04 H有（SE無）.mp3", duration=0.6)
    diff_2 = write_tiny_wav(folder / "TSSS 完パケ音声 mp3" / "差分トラック mp3" / "05 remix.mp3", duration=0.6)
    diff_3 = write_tiny_wav(folder / "TSSS 完パケ音声 mp3" / "差分トラック mp3" / "06 コピーしてお使いください.mp3", duration=0.6)

    plan = preprocess.plan_folder_input(folder)

    assert plan.status == "planned"
    assert plan.mix_parts == (
        main_1.resolve(),
        main_2.resolve(),
        diff_1.resolve(),
        diff_2.resolve(),
...
def test_folder_input_planner_silences_duplicate_effect_variants_for_asr(tmp_path: Path) -> None:
    folder = tmp_path / "RJVAR"
    base = write_tiny_wav(folder / "MP3" / "Append02_006.mp3", duration=0.7)
    off_se = write_tiny_wav(folder / "MP3" / "Append02_006_off_se.mp3", duration=0.7)
    on_ef = write_tiny_wav(folder / "MP3" / "EFFECT_ON" / "Append02_006_on_ef.mp3", duration=0.7)
    on_ef_off_se = write_tiny_wav(
        folder / "MP3" / "EFFECT_ON" / "Append02_006_on_ef_off_se.mp3",
        duration=0.7,
    )
    other = write_tiny_wav(folder / "MP3" / "MSOT_DL_04.mp3", duration=0.6)

    plan = preprocess.plan_folder_input(folder)

    assert plan.status == "planned"
    assert plan.asr_source_status == "mix_parts_silenced_duplicates"
    assert set(plan.mix_parts) == {
        base.resolve(),
        off_se.resolve(),
        on_ef.resolve(),
        on_ef_off_se.resolve(),
        other.resolve(),
    }
    assert plan.asr_parts == plan.mix_parts
    assert set(plan.asr_silent_parts) == {
        base.resolve(),
        on_ef.resolve(),
        on_ef_off_se.resolve(),
    }
    assert off_se.resolve() not in plan.asr_silent_parts
...
def test_full_command_runs_mock_e2e_with_project(cli_runner, tiny_wav_path: Path, tmp_path: Path) -> None:
    ...
    assert "synth: done=1/" in result.output
...
def test_full_command_batches_all_folder_groups_outputs(cli_runner, tmp_path: Path, monkeypatch) -> None:
    folder = tmp_path / "RJBATCH"
    write_tiny_wav(folder / "Main Pack" / "MP3" / "01 Main.mp3", duration=0.7)
    write_tiny_wav(folder / "Append Pack" / "MP3" / "02 Append.mp3", duration=0.8)
    write_tiny_wav(folder / "Bonus" / "03 Bonus.mp3", duration=0.6)
    project = tmp_path / "batch_project"
    ...
    assert result.exit_code == 0, result.output
    manifest = load_manifest(project)
    folder_manifest = json.loads(Path(manifest.artifacts["folder_input_manifest"]).read_text("utf-8"))
    export_manifest = json.loads(Path(manifest.artifacts["export_manifest"]).read_text("utf-8"))
    assert [Path(part["path"]).name for part in folder_manifest["mix_parts"]] == [
        "02 Append.mp3",
        "03 Bonus.mp3",
        "01 Main.mp3",
    ]
...
def test_full_real_applies_high_quality_preset_by_default(
    ...
    assert cfg.gsv_few_shot_min_total_sec == 60.0
    assert cfg.gsv_few_shot_min_clip_sec == 2.0
    assert cfg.gsv_few_shot_max_clip_sec == 8.0
    assert cfg.gsv_few_shot_min_quality_score == 0.30
    assert cfg.gsv_few_shot_max_background_bleed_db == -21.0
    ...
    assert cfg.gsv_tts_min_speed_factor == 0.90
    assert cfg.gsv_tts_max_speed_factor == 1.20
    assert cfg.gsv_timefit_max_tempo == 1.18
    assert cfg.gsv_timefit_max_stretch == 1.08
    assert cfg.gsv_timefit_micro_max_tempo == 1.30
    assert cfg.gsv_timefit_long_max_stretch == 1.15
    assert cfg.gsv_max_attempts_per_candidate == 3
    assert cfg.gsv_duration_rewrite_backend == "none"
    assert cfg.gsv_duration_rewrite_max_attempts == 1
    assert cfg.gsv_duration_rewrite_pre_candidate_count is None
    ...
    assert cfg.gemma_text_context_radius == 10
    assert cfg.gemma_text_concurrency == 1
    assert cfg.gemma_text_n_predict == 1536
    ...
    assert cfg.rvc_train_preferred_chars_per_sec == 4.5
    assert cfg.rvc_train_max_chars_per_sec == 5.2
