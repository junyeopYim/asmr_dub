...
def run_source_speakers_stage(
    ...
    if backend == "none":
        backend = "mock"
    _log_stage_start("source-speakers", f"backend={backend}, segments={len(manifest.segments)}")
    ...
    assign_source_speakers_to_manifest(project_dir, manifest, backend_kind=backend)
